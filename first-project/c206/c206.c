...
void DLL_Dispose( DLList *list ) {
	while(list->firstElement != NULL){
		DLLElementPtr tempPtr = list->firstElement->nextElement;
		free(list->firstElement);
		list->firstElement = tempPtr;
	}
	list->activeElement = NULL;
	list->lastElement = NULL;
}
...
void DLL_InsertAfter( DLList *list, int data ) {
	if(DLL_IsActive(list)){
		...
		DLLElementPtr next_element = list->activeElement->nextElement;

		list->activeElement->nextElement = new_element;
		new_element->previousElement = list->activeElement;
		new_element->nextElement = next_element;

		if(next_element != NULL){
			next_element->previousElement = new_element;
		}
		else{
...
void DLL_InsertBefore( DLList *list, int data ) {
	if(DLL_IsActive(list)){
		...
		DLLElementPtr previous_element = list->activeElement->previousElement;

		list->activeElement->previousElement = new_element;
		new_element->nextElement = list->activeElement;
		new_element->previousElement = previous_element;

		if(previous_element != NULL){
			previous_element->nextElement = new_element;
		}
		else{
